...
def store_temp(file_path, pdf_content):
    if not os.path.exists('documents'):
        os.makedirs('documents')
    with open(f'documents/{file_path}', 'wb') as f:
        f.write(pdf_content) 
...
def make_batch(folder_path, output_folder):
    ...
    for file_name in tqdm(source_files, desc="asking question to pdf for --> {file_name}"):
        ...
        if file_name.endswith(".pdf"):
            ...
            store_temp(file_name, open(file_path, 'rb').read())

            init_response = initialize_index(file_name)
            temp_dict = {}
            if init_response['status'] != 'success':
                return "Error with the indexing, please try again"

            else:
                temp_dict['id'] = id
                temp_dict['applicant'] = query_index("what is the name of applicant.write the name only")#ask_until_question(id, "what is the name of applicant")
                temp_dict['defendant'] = query_index("what is the name of defendant.write the name only")#ask_until_question(id, "what is the name of defendant")
                temp_dict['date'] = query_index("date of case")#ask_question(id, "date of case")
                temp_dict['case_number'] = query_index("what is the case number/adjucation number")#ask_until_question(id, "what is the case number/adjucation number")
                temp_dict['district'] = query_index("write name of district office")#ask_until_question(id, "write the name of the district office")
                temp_dict['decision'] = query_index("what is the final decision of the case")#ask_until_question(id, "what is the final decision of the case")
                temp_dict['case_name'] = query_index("write the name of the case")#ask_until_question(id, "write the name of the case")
                temp_dict['section_codes'] =query_index("name the section codes in the case") #ask_until_question(id, "name the section codes in the case")
                temp_dict['references'] = query_index("name all the references to other legal cases")#ask_until_question(id, "name the references in the case")
                temp_dict['summary'] = query_index("write a small summary of the case")#ask_question(id, "write a small summary of the case")
                try:
                    temp_dict['text'] = extract_text_from_pdf(file_path)
                    temp_dict['seo_keywords'] =  query_index("write 5 seo keywords for the case")#ask_question(id, "write 5 seo keywords for the case")
                except Exception:
                    temp_dict['text'] = ""
            ...
            bda_html_list = []
            for i in tqdm(range(num_files),desc="Now spliting the pdf for the html"):
            ...
            print(f"JSON data saved to {output_folder}/{file_ka_naam}.json")
            print("Uploading to GCS")
            clean_documents_folder()
            print("Cleaning complete of `documents` folder")
